...
type SyncSectionKey = "accountingAccounts" | "activeJobId" | "billing" | "customers" | "fieldNotes" | "fieldProgress" | "inventoryLocations" | "jobs" | "materials" | "objects" | "packages" | "personnel" | "portalMessages" | "reports" | "resources" | "services" | "translationOverrides";
...
const englishUiText: Record<string, string> = {
  ...
  "Abgeschlossene Berichte": "Completed reports",
  "abrechenbar": "billable",
  "aktive Objekte": "active properties",
  "Aktualisiere": "Updating",
  "Aktualisieren": "Refresh",
  "Alle Tage": "All days",
  "An": "To",
  "Angelegt am": "Created on",
  "Auftrag bearbeiten": "Edit job",
  "Auftrag speichern": "Save job",
  "Auftragsbeschreibung": "Job description",
  "Bearbeiter": "Editor",
  "Beispiel: Privat|https://calendar.google.com/calendar/ical/.../basic.ics": "Example: Private|https://calendar.google.com/calendar/ical/.../basic.ics",
  "Betreff": "Subject",
  "Bild hinzufügen": "Add image",
  "Bilder zur Ressource": "Images for resource",
  "Deine Ferienhäuser": "Your holiday homes",
  "Deine offenen Aufträge": "Your open jobs",
  "Deine Stammdaten": "Your master data",
  "Dunkelmodus": "Dark mode",
  "Eigene Leistung erfassen": "Enter custom service",
  "Einsatz abgeschlossen": "Job completed",
  "Einsatzbericht": "Job report",
  "Einsatzberichte": "Job reports",
  "Einsatzplanung": "Job planning",
  "Enddatum": "End date",
  "Ende": "End",
  "Endet am": "Ends on",
  "Fahrt erfassen": "Record trip",
  "Fahrtenbuch Jahr": "Logbook year",
  "Finanzen": "Finance",
  "Format pro Zeile: Name|ICS-Link": "Format per line: Name|ICS link",
  "Geburtstagskalender": "Birthday calendar",
  "Hellmodus": "Light mode",
  "Heute": "Today",
  "Hinweis / Info": "Note / info",
  "Historie / Verlauf": "History / timeline",
  "Interne Notizen": "Internal notes",
  "Kacheln": "Tiles",
  "Kalender heute plus 3 Tage": "Calendar today plus 3 days",
  "Kalenderquellen konfigurieren": "Configure calendar sources",
  "Kategorie": "Category",
  "Km-Stand Jahresbeginn": "Odometer at year start",
  "Km-Stand Jahresende": "Odometer at year end",
  "Kommunikation": "Communication",
  "Kontakt und Rechnungsadresse": "Contact and billing address",
  "Kundenportal": "Customer portal",
  "Kundensichtbar": "Customer visible",
  "Kundenübersicht": "Customer overview",
  "Leistung anfragen": "Request service",
  "Leistungen im Auftrag": "Services in job",
  "Liste": "List",
  "Mailtext Einsatzbericht": "Job report email text",
  "Materialbestand, Lagerorte, Ein- und Ausgänge sowie Einkaufsbelege zentral verwalten.": "Manage material stock, storage locations, stock in/out and purchase receipts centrally.",
  "Mobil vor Ort": "Mobile field work",
  "Nachricht an Kolaretorp Service AB": "Message to Kolaretorp Service AB",
  "Nachrichtenverlauf": "Message history",
  "Nächster Besuch": "Next visit",
  "Neue Ressource anlegen": "Create new resource",
  "Neuer Kunde": "New customer",
  "Neues Personal anlegen": "Create new personnel",
  "Objekt bearbeiten": "Edit property",
  "Objekte, Einsätze, Berichte und Abrechnung in einer Arbeitszentrale.": "Properties, jobs, reports and billing in one workspace.",
  "offene Einsätze": "open jobs",
  "Passwort": "Password",
  "Personal": "Personnel",
  "Personal als Kacheln anzeigen": "Show personnel as tiles",
  "Personal als Liste anzeigen": "Show personnel as list",
  "Personalnummer": "Personnel number",
  "Preis": "Price",
  "Priorität": "Priority",
  "Rechnungen": "Invoices",
  "Rechnungsübersicht": "Invoice overview",
  "Ressourcen als Kacheln anzeigen": "Show resources as tiles",
  "Ressourcen als Liste anzeigen": "Show resources as list",
  "Rhythmus": "Rhythm",
  "Rolle": "Role",
  ...
  "Zeit": "Time",
  "Zeit / Material": "Time / material",
  "Zusammenfassung": "Summary",
  "Zuständig": "Responsible",
  "Zweck / Ärende": "Purpose / matter",
};
...
function translationFileRows(overrides: TranslationFileRow[] = []) {
  ...
  Object.entries(swedishUiText).forEach(([key, sv]) => upsert(key, {
    de: key,
    ...(englishUiText[key] ? { en: englishUiText[key] } : {}),
    sv,
  }));
...
function recoverReportsFromFieldProgress(snapshot: AppSnapshot): AppSnapshot {
  const deletedReportIds = new Set(snapshot.deletedReportIds ?? []);
  const sourceReports = snapshot.reports.filter((report) => !deletedReportIds.has(report.id));
  const existingReportKeys = new Set(sourceReports.map((report) => `${report.jobId}|${normalizeReportDate(report.date)}`));
  const repairedReports = sourceReports.map((report) => {
  ...
  const recoveredReports = snapshot.jobs.flatMap((job) => {
    ...
    return progressEntries.flatMap(({ date, progress }) => {
      ...
      if (!["erledigt", "abgerechnet"].includes(job.status)) return [];
...
function patchUsesSmallSyncOnly(overrides: Partial<AppSnapshot>) {
  ...
  return keys.length > 0 && keys.every((key) => key === "updatedAt" || ["accountingAccounts", "activeJobId", "billing", "customers", "fieldNotes", "fieldProgress", "inventoryLocations", "jobs", "materials", "objects", "packages", "personnel", "portalMessages", "reports", "resources", "services", "translationOverrides"].includes(key));
}
...
async function downloadMaterialInventoryMovementPdf(material: MaterialItem, customers: CustomerRecord[], services: ServiceItem[]) {
  ...
  const columns = [
    { label: "Datum / Uhrzeit", width: 30 },
    { label: "Buchung", width: 22 },
    { label: "Lagerort", width: 32 },
    { label: "Menge", width: 34 },
    { label: "Bestand", width: 26 },
    { label: "Wert", width: 27 },
    { label: "Lieferant / Kunde", width: 42 },
    { label: "Notiz", width: contentWidth - 213 },
  ];
...
function InventoryView({
  ...
  async function captureInventoryReceipt(file: File) {
    setNotice("Einkaufsbeleg wird gespeichert...");
    try {
      const attachment = await fileToReportAttachment(file);
      setForm((current) => ({ ...current, receipt: attachment }));
      setNotice("Einkaufsbeleg wurde zur Buchung hinzugefügt.");
    } catch (error) {
      console.warn("Einkaufsbeleg konnte nicht gespeichert werden.", error);
      setNotice("Einkaufsbeleg konnte nicht gespeichert werden.");
    }
  }
  ...
  function openHistory(material?: MaterialItem) {
    setHistoryMaterialId(material?.id ?? null);
    setHistoryOpen(true);
  }
      ...
      <div className="table-list compact-list inventory-overview-list">
        {filteredMaterials.map((material) => {
          ...
          const locationSummary = Object.entries(materialInventoryByLocation(material.inventoryEntries ?? []))
            ...
            .join(" · ");
          return (
            <article className={selectedMaterial?.id === material.id ? "active" : ""} key={material.id} onClick={() => setSelectedMaterialId(material.id)}>
              <div>
            ...
            <div className="form-grid compact-form inventory-booking-form">
              ...
              {form.receipt && (
                <div className="wide inventory-receipt-note">
                  ...
                  <button className="icon-button" aria-label="Einkaufsbeleg entfernen" onClick={() => setForm({ ...form, receipt: undefined })} type="button"><Trash2 size={15} /></button>
                </div>
              )}
            </div>
        ...
        <div className="modal-backdrop nested-backdrop">
          <section aria-labelledby="inventory-history-title" aria-modal="true" className="modal send-preview-modal catalog-editor-modal" role="dialog">
            ...
            <div className="material-inventory-history">
              ...
              {historyEntries.length === 0 && <p>Noch keine Lagerbuchungen vorhanden.</p>}
            </div>
          </section>
        </div>
      )}
